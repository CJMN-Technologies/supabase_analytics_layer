...
- **Events Consolidated (`external.events_consolidated`):**
  ...
  - **Retrospective Photo Recap Guardrail**: If a Facebook post has `event_code = 'MAJOR_ARENA_EVENT'` and its LLM-extracted `event_date` is **more than 1 day before** the post publication date (`post_date`), the trigger treats the post as a past-event photo album or celebratory recap and silently discards it from `events_consolidated` without raising an error. The same logic applies if retrospective phrasing is detected in the post text (`\"naging matagumpay\"`, `\"photo highlight\"`, `\"event recap\"`, `\"successfully held\"`, etc.). This prevents social media photo recaps published days after an event from creating false active disruption shocks in the analytics layer. Complementary pipeline-level guard (Layer 4) pre-screens posts before database insertion. Applied retroactively via `Analytics/sql/fix_retrospective_recap_guardrail.sql` (Sept 14, 2026).
