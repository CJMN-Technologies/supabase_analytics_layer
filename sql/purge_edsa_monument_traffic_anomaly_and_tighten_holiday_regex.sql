-- ============================================================================
-- PURGE EDSA MONUMENT TRAFFIC ANOMALY & TIGHTEN HOLIDAY REGEX
-- Target Supabase Project: kthioobzfyepokrrykem
-- Execution Date: 2026-09-21
--
-- 1. Purge 5 spurious holiday shock records from external.events_consolidated:
--    - external_lgu_0296 (QC Gov EDSA People Power Monument traffic advisory)
-- 2. Mark source record in external.academic_lgu_events as is_cancelled = true
-- 3. Harden external.classify_event_from_text():
--    - Replace loose \bedsa\b with \bedsa\s+(?:people\s+power\s+)?(?:day|revolution|anniversary)\b
--    - Add motorist surface traffic advisory non-disruptive filter
-- 4. Harden external.sync_academic_lgu_to_events_consolidated():
--    - Prevent CIVIC_MAINTENANCE / WEATHER_ADVISORY from triggering statutory holidays
--    - Suppress non-disruptive motorist road traffic advisories from train ridership shock table
-- ============================================================================

BEGIN;

-- Step 1: Purge spurious records from external.events_consolidated
DELETE FROM external.events_consolidated
WHERE source_id = 'external_lgu_0296' OR id LIKE 'SCR-HD-0921-external_lgu_0296-%';

-- Step 2: Mark source record in external.academic_lgu_events as cancelled / audit-logged
UPDATE external.academic_lgu_events
SET is_cancelled = TRUE,
    cancellation_reason = 'Motorist traffic advisory near EDSA People Power Monument erroneously matched unconstrained edsa holiday keyword; does not affect transit ridership.'
WHERE id = 'external_lgu_0296';
...
BEGIN
    ...
    -- Filter 5a: Statutory, National, LGU, and University Holidays (Constrained regex for araw ng & EDSA)
